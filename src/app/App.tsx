...
import { getBookData, getBookTitle } from './data/bookPages';  // NEW: Import book data helper
      ...
      case 'bookReader':
        return (
          <BookReader
            ...
            bookTitle={getBookTitle(selectedBookId)}
            bookContent={getBookData(selectedBookId)}
            onBack={() => setCurrentScreen('bookOverview')}
          />
        );
      case 'immersiveReader':
        const bookData = getBookData(selectedBookId);
        return (
          <ImmersiveBookReader
            user={user}
            setUser={setUser}
            theme={currentTheme}
            bookId={selectedBookId}
            bookTitle={bookData?.title || getBookTitle(selectedBookId)}
            bookContent={bookData?.pages || []}
