...
import { useCharacters, useBooks } from '../hooks/useAssets';
...
// The Character type is taken from the hook – we don't need to redeclare it.
// The games expect a Character object with at least id, name, image, category, description, fact.
// The hook returns objects that satisfy that.
...
export function MiniGames({ user, setUser }: MiniGamesProps) {
  ...
  const isLoading = charsLoading || booksLoading;
...
// Maze Hunt Game Component - Imported from MazeHuntGameRedesigned.tsx
  ...
  const generateQuestions = () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const wrongAnswers = characters
        .filter((c: any) => c.id !== randomChar.id)
            ...
            <div className="text-center p-6">
              ...
              >
                <img
                  ...
                  className={`w-full h-full object-cover transition-all duration-500 ${selectedAnswer === questions[currentQuestion].correct
                    ? 'blur-none'
                    : 'blur-xl'
                    }`}
                />
                  ...
                  return (
                    ...
                    >
                      <Button
                        ...
                        className={`h-16 w-full text-lg font-semibold transition-all ${buttonClass} ${isSelected && !showResult ? 'ring-4 ring-blue-300' : ''
                          }`}
...
function WordPuzzleGame({ characters, onBack, onScore, gameHeader, gameControls, isPaused, onNextGame }: any) {
  ...
  useEffect(() => {
    // Select a random character when the game starts
...
function MemoryMatchGame({ characters, onBack, onScore, gameHeader, gameControls, isPaused, onNextGame }: any) {
  const [cards, setCards] = useState<Array<{ char: any; flipped: boolean; matched: boolean; id: number }>>([]);
              ...
              {cards.map((card, index) => (
                ...
                >
                  <div
                    ...
                    className={`relative w-full h-full rounded-xl cursor-pointer transition-all transform ${card.matched ? 'opacity-50 scale-95' : 'hover:scale-105'
                      }`}
                  >
                    <AnimatePresence mode="wait">
                      {card.flipped || card.matched ? (
                        ...
                        >
                          <div className={`w-full h-full rounded-xl overflow-hidden border-4 ${card.matched ? 'border-green-500' : 'border-purple-500'
                            }`}>
                            <img
                              src={card.char.image}
                              alt={card.char.name}
